...
with jsonlines.open('paronyms/paronyms_alisa_new.jsonl') as f:
    paronyms = pandas.DataFrame(f)

# Загрузка сообщений
with jsonlines.open('accents/accents_alisa.jsonl') as f:
    accents = pandas.DataFrame(f)
...
def start_message(message):
    ...
    if str(message.chat.id) not in history.keys():
        ...
        history[str(message.chat.id)]["Successful"] = dict.fromkeys(typeofQ, [])
        history[str(message.chat.id)]["Easy"] = dict.fromkeys(typeofQ, [])
        ...
        history[str(message.chat.id)]["Type"] = 0   # 0 Orphography, 1 Paronyms, 2 Accents
...
def quiz(message):
    ...
    acc_button=types.InlineKeyboardButton(text="Ударения", callback_data="accents")
    start_quiz.add(orpho_button, paro_button, acc_button)
...
def question(call):
    ...
    history[str(call.message.chat.id)]["Now"] = random.choice(list(all_questions - set(history[str(call.message.chat.id)]["Successful"][typeofQ[history[str(call.message.chat.id)]["Type"]]]) - set(history[str(call.message.chat.id)]["Easy"][typeofQ[history[str(call.message.chat.id)]["Type"]]])))
    write_history()
...
def callback_inline(call):
    ...
    if call.message:
        if call.data[0] == "T" or call.data[0] == "F":
            ...
            if call.data[0] == "T":
                read_history()
                history[str(call.message.chat.id)]["Successful"][typeofQ[history[str(call.message.chat.id)]["Type"]]].append(history[str(call.message.chat.id)]["Now"])
            ...
            read_history()
            history[str(call.message.chat.id)]["Easy"][typeofQ[history[str(call.message.chat.id)]["Type"]]].append(history[str(call.message.chat.id)]["Now"])
            write_history()
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=call.message.text)
